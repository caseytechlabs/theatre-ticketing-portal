...
const STORAGE_KEY = 'theater_booking_ids'
...
export function BookingsPage() {
  ...
  const handleConfirm = async (success: boolean) => {
    ...
    try {
      const result = await bookingApi.confirm({
      ...
      notify(
        success
          ? 'Payment confirmed — voucher has been claimed!'
          : 'Payment declined — voucher released back to pool.'
      )
    } catch (e: any) {
  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="text-gray-500 mt-1">Look up and manage ticket bookings</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-20 text-gray-400">
          ...
          <p className="text-lg font-medium">No bookings tracked yet</p>
          <p className="text-sm mt-1">Initiate a booking from the Vouchers page, then paste the ID above</p>
        </div>
      ) : (
        ...
        <div className="space-y-4">
          <div className="p-4 bg-gray-50 rounded-xl text-sm text-gray-600 space-y-1 border border-gray-100">
            <p>
              ...
              <span className="font-mono text-xs break-all">{confirmTarget?.id}</span>
            </p>
            <p>
              <span className="text-gray-400">Voucher</span>{' '}
              <span className="font-mono text-xs break-all">{confirmTarget?.voucherId}</span>
            </p>
            <p>
              <span className="text-gray-400">User</span>{' '}
              <span className="font-medium">{confirmTarget?.userId}</span>
            </p>
          </div>

          <p className="text-sm text-gray-700 font-medium">Did the payment succeed?</p>
